...
from optimization_models.optimization_model_v2.mnistfnn import MNISTFNN
from optimization_models.model_consts import RunType
from optimization_models.optimization_model_v2.utils import TrialParam
...
class ArchitectureSampler:
    ...
    def _suggest_params(self, trial: optuna.Trial) -> None:
        trial.suggest_categorical(TrialParam.DATAWIDTH.value, [8, 16])
        trial.suggest_int(TrialParam.HEAD_DEPTH.value, 0, self.architecture.max_head_depth)
        trial.suggest_int(TrialParam.TAIL_DEPTH.value, -1, self.architecture.max_tail_depth)
        ...
        trial.suggest_int(TrialParam.BOTTLENECK_WIDTH.value, 1, self.architecture.max_bn_width)
    ...
    def _build_trial_layers(self, trial: optuna.Trial) -> Tuple[List[int], List[int]]:
        head_depth = trial.params[TrialParam.HEAD_DEPTH.value]
        tail_depth = trial.params[TrialParam.TAIL_DEPTH.value]
        ...
        if head_depth > 0 and tail_depth > 0:
            head_layers[-1] = trial.params[TrialParam.BOTTLENECK_WIDTH.value]
    ...
    def build_model(self, trial:optuna.Trial) -> MNISTFNN:
        self._suggest_params(trial)
        datawidth = trial.params[TrialParam.DATAWIDTH.value]
        head_layers, tail_layers = self._build_trial_layers(trial)
        runType = self._get_trial_runType(trial)

        return MNISTFNN(head_layers, tail_layers, datawidth, runType)
    
    def recover_model(self, trial:optuna.Trial) -> MNISTFNN:
        datawidth = trial.params[TrialParam.DATAWIDTH.value]
        head_layers, tail_layers = self._build_trial_layers(trial)
        runType = self._get_trial_runType(trial)
