...
from optimization_models.model_consts import RunType, shapeType, Hardware
...
class ModelStats():
    def __init__(self, model: MNISTFNN, hardware:Hardware):
        self.model = model
        self.hw = hardware

        self.head_stats = PrelimModuleStats(model.head, model.head_shape, self.hw)
        ...
        self.latency_s = self._get_latency()
    ...
    def _get_latency(self) -> float:
        ...
        if self.model.runType == RunType.EDGE_COMPUTING:
            head_latency_s = self.head_stats.cycles / self.hw.edge_clock_Hz
            tail_latency_s = self.tail_stats.cycles / self.hw.cloud_clock_Hz
        else:
            head_latency_s = (self.head_stats.cycles + self.hw.cycles_per_tx) / self.hw.edge_clock_Hz
            tail_latency_s = (self.tail_stats.cycles + self.hw.cycles_per_rx) / self.hw.cloud_clock_Hz

        tx_latency_s = self.transmission_size_bit / self.hw.bandwidth_bps

        return head_latency_s + tx_latency_s + tail_latency_s

    def _get_power(self) -> float:
        power_pW = (self.head_stats.energy_pJ + self.hw.tx_energy_per_bit_pJ * self.transmission_size_bit) / self.latency_s
        return power_pW
